  ...
  if (this.config.iamRole) {
    if (this.config.iamRole == 'any') {
      ...
      ms.refresh(function(err) {
        if (err) { this.log('Failed to fetch IAM role credentials: ' + err, LogLevels.ERROR); }
  ...
  if (currentMetricsBatch.length > 0) {
    ...
    for (var i = 0, len = chunkedGroups.length; i < len; i++) {
      this.cloudwatch.putMetricData({
        ...
        if (err) {
          this.log(util.inspect(err), LogLevels.ERROR);
        } else {
          this.log(util.inspect(data), LogLevels.VERBOSE);
        }
      });
    }
  }
