...
CloudwatchBackend.prototype.graphcoolNamespaceProcessing = function(key) {
  ...
  console.log('namespace processing! ' + key)
    ...
    if (metrics.length > 0) {
      ...
      for (var i = 0, len = chunkedGroups.length; i < len; i++) {
        self.cloudwatch.putMetricData({
          ...
          if (err) {
            self.log(util.inspect(err), LogLevels.ERROR);
          } else {
            self.log(util.inspect(data), LogLevels.VERBOSE);
          }
        });
      }
    }
...
CloudwatchBackend.prototype.flush = function(timestamp, metrics) {
  ...
  this.log(util.inspect(metrics), LogLevels.VERBOSE);
  ...
  for (key in counters) {
    ...
    console.log("calculated namespace: " + namespace);
  ...
  console.log('namespace to take: ' + namespace);
...
exports.init = function(startupTime, config, events) {
  ...
  console.log("Starting with configuration: " + util.inspect(cloudwatch))
